...
void GenMesh::ApplyDataToGL(const unsigned int* a_indices)
{
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_indexCount * sizeof(GLuint), a_indices, GL_STATIC_DRAW);

	// Unbind VAO (memsave)
	glBindVertexArray(0);
}

void GenMesh::GenSegmentedGrid(const unsigned int a_dim,
	const float a_segScale,
...
void GenMesh::GenQuad(glm::vec4 a_pos, float a_scale)
{
	m_dims = 2;
	...
	for (GLuint r = 0; r < m_dims; ++r)
	{
		...
		{
			verts[r * m_dims + c].position = a_pos;
			verts[r * m_dims + c].position += glm::vec4((float)(c * a_scale), (float)(r * a_scale), 0, 1);
