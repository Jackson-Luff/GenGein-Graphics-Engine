...
SProgram::~SProgram()
{
	...
	delete m_fragShader;
	delete m_geomShader;
	delete m_tessCShader;
	delete m_tessEShader;
}
...
bool SProgram::AttachShadersFromDir(const char* a_folderDir, const char* a_comName)
{
	...
	m_geomShader = &GenShader(std::string(dir + ".geom"), ShaderID::GEOM_SHADER);
	m_tessCShader = &GenShader(std::string(dir + ".tessC"), ShaderID::TESSC_SHADER);
	m_tessEShader = &GenShader(std::string(dir + ".tessE"), ShaderID::TESSE_SHADER);
