...
bool Shader::AttachToProgram(unsigned int a_program)
{
	if (m_shaderID == -1)
	{
		Console::Log("#ERR | ATTACH FAILED. %s IS AN INVALID SHADER\n", m_directory.c_str());
		return false;
	}
...
bool Shader::CreateShader(std::string a_src, ShaderType a_shaderType)
{
	...
	{
		Console::Log("#ERR | Shader directory %s invalid.\n", m_directory.c_str());
		return false;
	}
	else
	{
...
std::string Shader::ExtractFromFile(std::string a_filePath)
{
	...
	if (!src)
	{
		Console::Log("#ERR | Could not read file %s. File does not exist\n", a_filePath.c_str());
		return std::string();
	}
